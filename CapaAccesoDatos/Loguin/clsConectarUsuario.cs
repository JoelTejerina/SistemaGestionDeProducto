...
namespace CapaAccesoDatos
{
...

    {
        public bool Login(string user, string pass)
        {
           ...
           " where Usuarios.usuario= '" + user + "' and Usuarios.Password= '" + pass + "' and Usuarios.UsuarioDesactivado=false";
        ...
        public bool UserVerific(string user)
        {
            ...
            if (DT2.Rows.Count > 0)
            {
                ...
                {
                    UserCache.IdUsuario = Convert.ToInt32(row2[0].ToString());
                    UserCache.Usuario = row2[1].ToString();
                    UserCache.Apellido = row2[3].ToString();
                    UserCache.Nombres = row2[4].ToString();
                }
                return true;
            }
            else
